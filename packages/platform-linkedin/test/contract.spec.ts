...
import { linkedin } from '../src/index.js';
...
function createHarness(): ContractHarness {
  ...
  return {
    platform: linkedin.create({ logger: silentLogger }),
    accountConfig,

    respondSuccess() {
      respond(
        () =>
          new Response(JSON.stringify({ id: '1', url: 'https://linkedin.example/1' }), {
            status: 200,
...
describePlatformContract({
  ...
  requests: {
    ...
    [PostType.IMAGE]: {
      ...
      target: { id: 'urn:li:person:test' },
      type: PostType.IMAGE,
      media: [{ type: 'image', source: { kind: 'platformRef', ref: 'urn:li:image:1' } }],
    },
    [PostType.VIDEO]: {
      platform: 'linkedin',
      target: { id: 'urn:li:person:test' },
    ...
    [PostType.DOCUMENT]: {
      ...
      media: [{ type: 'document', source: { kind: 'platformRef', ref: 'urn:li:document:1' } }],
    },
  },

  overLimitRequest: {
    request: { platform: 'linkedin', body: 'x'.repeat(3001), type: PostType.POST },
