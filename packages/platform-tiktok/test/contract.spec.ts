...
import { tiktok } from '../src/index.js';
...
function createHarness(): ContractHarness {
  ...
  return {
    platform: tiktok.create({ logger: silentLogger }),
    accountConfig,

    respondSuccess() {
      respond(
        () =>
          new Response(JSON.stringify({ id: '1', url: 'https://tiktok.example/1' }), {
            status: 200,
...
describePlatformContract({
  ...
  overLimitRequest: {
    request: { platform: 'tiktok', type: PostType.ALBUM, media: [] },
    expectedError: /media/i,
  },
