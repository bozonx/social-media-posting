...
export class TelegramAuthValidator implements IAuthValidator {
  ...
  validate(auth: Record<string, unknown>): AuthValidation {
    return validateTelegramAuth(auth);
  }
}

/** Validate Telegram auth without constructing an adapter or performing I/O. */
export function validateTelegramAuth(auth: Record<string, unknown>): AuthValidation {
  const errors: string[] = [];
...
function isValidBotToken(token: string): boolean {
  return /^\d{6,12}:[A-Za-z0-9_-]{10,}$/.test(token);
}
