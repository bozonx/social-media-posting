...
**Что уже сделано:** локальная реализация волн 0–7 закрыта в обоих репозиториях; все целевые
адаптеры реализованы, а BloggerDog использует неймспейсированные platform options и
capability-driven `adaptedRequest` без платформенных request-форматтеров. Не закрыты только
внешние release-гейты: live smoke, доступы приложений и юридическое подтверждение Truth Social.
...
| 7     | консолидация библиотеки и BloggerDog | ✅ Локальная реализация завершена        | завершённые целевые адаптеры                 |
...
**Результат реализации.** Добавлен пакет `packages/platform-mastodon`, который экспортирует
Mastodon и полученный через `deriveModule()` Pixelfed с общим протокольным кодом. Реализованы
`/api/v2/instance`, загрузка медиа и alt text, статусы, опросы, ответы, явные треды и
...
После реализации адаптеров выполнено:

- ✅ завершена волна D в BloggerDog;
- ✅ удалён Telegram-specific request formatter; окончательный payload строится через
  capability-driven `adaptRequest()` — тот же механизм, который возвращает
  `preview().adaptedRequest`;
- ✅ проведён общий аудит scopes, логирования секретов, delivery semantics и platform matrix;
- ✅ пройдены локальные `pnpm validate` и `pnpm validate:all`;
- ✅ обновлены changelog и постоянная документация.

Ручные live smoke не объявляются пройденными без внешних аккаунтов и approvals. Они остаются
release-гейтами конкретных платформ и не являются незавершённой локальной реализацией волны 7.
...
### Волна D — консолидация хоста ✅

`platformOptions` хранится неймспейсированно по платформе. BloggerDog строит только общий
platform-neutral request из снапшота, target, auth и options, после чего сохраняет результат
библиотечного `adaptRequest()` по capability зарегистрированного адаптера. Отдельная
Telegram-стратегия удалена; ignored fields, конверсия/усечение body, выбор publish type и
доставка отдельных tags больше не дублируются на стороне хоста.
