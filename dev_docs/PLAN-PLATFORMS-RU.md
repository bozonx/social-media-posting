...
Pinterest, Mastodon, Bluesky и Vimeo. Адаптеры используют только Web APIs (`fetch`, `Request`, `Response`, WHATWG streams,
Web Crypto) и прямые HTTP-запросы. SDK поставщиков и runtime-зависимости запрещены.

WhatsApp Channels в этот список **не входит**: публичного API для публикации обновлений канала
не существует. Обоснование — в разделе «WhatsApp Channels — адаптер не планируется».
...
| WhatsApp Channels      | —     | —      | —                     | —                                      | —                 | —                                        | публичного API нет           |
...
фактическое значение берётся из `resolveCapabilities()`.
...
### WhatsApp Channels — адаптер не планируется
...
## Проверка ядра на сетях следующей волны

Эти сети не планируются к реализации. Они использованы как проверочные: если сеть описывается без
изменений ядра — ядро готово; если требует правку — эту правку нужно сделать сейчас, пока
адаптеров мало.

| Сеть             | Ложится на ядро? | Что именно проверяет                                                                         |
| ---------------- | ---------------- | -------------------------------------------------------------------------------------------- |
| **Dailymotion**  | Да, без правок   | Ещё одна видео-сеть с OAuth2 и загрузкой файла. Полностью повторяет форму YouTube/Vimeo.     |
| **PeerTube**     | Да, после п. 13  | Federated video: свой хост у каждого инстанса, resumable upload, обязательный выбор канала.  |
| **Pixelfed**     | Да, после п. 22  | Mastodon-совместимый API. Отдельный пакет не нужен — тот же код, другой дескриптор.          |
| **Truth Social** | Технически да    | Форк Mastodon с совместимым API. Ограничение не техническое, а условия использования/доступ. |

Вывод по проверке: **ни одна из четырёх не требует нового понятия в ядре сверх пунктов 13 и 22.**
Это и есть подтверждение, что список изменений ядра достаточен, а не бесконечен.

Практические выводы:

- **Dailymotion** — самый дешёвый адаптер во всём списке, но и самая маленькая аудитория.
  Делать по запросу, не по инициативе.
- **PeerTube** — сочетает две уже оплаченные механики (per-instance хост + resumable video). Из
  четырёх это единственный кандидат с самостоятельной ценностью: он даёт независимую от
  корпораций площадку для видео.
- **Pixelfed** и **Truth Social** — не проекты, а конфигурация. Если пункт 22 сделан, каждая из
  них — это дескриптор возможностей и запись в реестре. Если пункт 22 **не** сделан, каждая из
  них — копия пакета Mastodon, и через год их три и они разъехались. Это самый наглядный аргумент
  за то, чтобы сделать пункт 22 до Mastodon, а не после.
- По **Truth Social** решение принимается не инженерами: нужно подтверждение, что автоматическая
  публикация разрешена условиями сервиса и доступ к API получаем законно. Без этого пакет не
  выпускается, даже если он состоит из двадцати строк.

## Границы сервиса: чаты, форумы, стриминг

Вопрос «а стоит ли вообще» полезнее решать не по каждой сети, а один раз — критерием. Контракт
...
| Кандидат     | Условия     | Решение                                                                 |
| ------------ | ----------- | ----------------------------------------------------------------------- |
| **Discord**  | 1 ✓ 2 ✓ 3 ✓ | **Да.** Дешёвый и полезный, но как канал анонсов, а не как «соцсеть».   |
| **Reddit**   | 1 ✓ 2 ✓ 3 ✓ | **Да, но поздно.** Технически подходит, продуктово — самый рискованный. |
| **Snapchat** | 1 ~ 2 ✓ 3 ✓ | **Нет.**                                                                |
| **Twitch**   | 1 ✗ 2 ✓ 3 ✗ | **Нет.**                                                                |

### Discord — да, и это лучший вклад на единицу усилий

Discord формально мессенджер, но по механике проходит все три условия: сообщение в канале имеет
постоянный идентификатор и permalink, адресовано сообществу, а не человеку, и готовится заранее.

Почему это выгодно именно для BloggerDog: у большинства проектов, ведущих блог, есть Discord
сообщества, и анонс новой публикации туда — реальный, часто ручной сценарий. При этом стоимость
интеграции минимальна: webhook — один `POST` без OAuth, без review, без асинхронной обработки.
Это единственная сеть в списке, которую можно выпустить за день.

Что учесть: две разные модели доступа (webhook против bot token с правами в гильдии) — то есть
`auth.kind` различается внутри одной сети и должен выбираться конфигурацией аккаунта; лимит
размера вложения зависит от boost-уровня сервера, то есть снова динамическая capability
(пункт 14); embeds — платформенный формат, ложащийся на `extra`; `target` составной
(guild + channel, пункт 12). Ни одно из этих требований не новое.

Честная оговорка: это distribution, а не публикация в соцсеть. В интерфейсе BloggerDog его
стоит показывать как «анонс», иначе пользователь будет ждать от него охватов и статистики,
которых там нет.

### Reddit — да, но последним и с ограничениями

По механике подходит идеально: submission имеет URL, бывает text/link/image/video, у него есть
обязательный `title` — как раз тот случай, ради которого в ядре есть отдельное поле `title`, а
не только `body`. Правила сабреддита и обязательный flair нужно получать перед отправкой — это
ровно тот сценарий, ради которого вводится `resolveCapabilities()` (пункт 14), и Reddit будет его
лучшей проверкой.

Почему поздно, а не сейчас:

- автоматический постинг в чужие сабреддиты — самый быстрый способ получить бан аккаунта и
  репутационную проблему у продукта. Продуктово это безопасно только для **своего** сабреддита;
- правила и требования различаются от сабреддита к сабреддиту сильнее, чем что-либо ещё в этом
  списке; отказ «до сетевого вызова» здесь принципиально невозможен без предварительного запроса;
- условия доступа к API менялись резко и могут меняться снова.

Рекомендация: планировать после того, как `resolveCapabilities()` отработан на TikTok и Mastodon,
и с явным ограничением сценария до собственных сообществ.
...
публикаций с историей и статистикой, это чужая модель. Оставить в каталоге как `restricted` и не
трогать.

### Twitch — нет, и это не вопрос приоритета
...
**чтение** чужого API и триггер, а не публикация; такой функциональности место в BloggerDog как
источнику события, а не здесь.
...
добавляется, когда проходит три условия выше **и** есть пользователь, который её просил. Каталог
существует именно для того, чтобы «мы знаем про эту сеть» не означало «мы её поддерживаем».
...
2. **Изменения ядра под будущие сети (пункты 12–23), пока адаптер один.** Структурный `target`,
...
9. Реализовать `platform-mastodon` — приёмочный тест федеративной модели: `apiBaseUrl`,
   `resolveCapabilities()` из `/api/v1/instance`, per-instance OAuth client, треды и
   `Idempotency-Key`. Сразу же вывести из него дескриптор Pixelfed через механизм пункта 22 —
   именно это доказывает, что механизм работает, а не просто существует.
10. Реализовать `platform-bluesky` с отдельно протестированным построением `facets` по
    UTF-8-байтам и счётчиком графем.
11. Реализовать `platform-vimeo`, переиспользовав resumable-механику YouTube; расхождение —
    протокол tus и квота хранилища вместо суточной квоты операций.
12. Для каждого пакета: README, auth validator, capabilities с official sources, HTTP/error layer,
    adapter, recorded fixtures, unit tests, conformance tests и workerd tests.
13. Подключить пакеты в root tsconfig/publish scripts/server Docker workspace и `PLATFORMS`,
...
14. Выполнить `pnpm validate`; перед выпуском — `pnpm validate:all`, sandbox accounts и ручные
...
готово, и это дешевле узнать до шага 4, чем после восьмого адаптера.

Порядок 9–11 намеренно ставит Mastodon после Meta-сетей, а не первым: контейнерные flow дают
больше нового кода, а Mastodon проверяет ядро — и проверка полезнее, когда есть что проверять.
