...
**Что уже сделано:** ядро (пункты 1–23) закрыто; выпущен `packages/platform-discord`; на стороне
`../bloggerdog` выполнена волна A и удалён VK. Подробности — в разделах «Порядок реализации» и
...
Статус: **группа 0 и волна A выполнены** (30 августа 2026). Ядро (пункты 1–23) закрыто до
первого нового адаптера, как и планировалось.
...
Такая группировка даёт три вещи, которых не даёт группировка по контенту: внутри группы второй и
третий адаптер стоят в разы дешевле первого; каждая группа проверяет конкретный кусок ядра, и
если ядро не выдержало — это видно на первом адаптере группы, а не на восьмом; сети с
многонедельным получением доступа вынесены в отдельную группу и не блокируют конвейер.

**Ядро (сделано, до первого нового адаптера)**
...
**Параллельно, а не после:** подать заявки на доступ к TikTok, X и LinkedIn и запросить
подтверждение условий по Truth Social. Сроки измеряются неделями, и это единственная зависимость,
которую нельзя ускорить кодом (см. решение 6).

**Группа 0 — сквозной путь (сделано)**
...
**Группа 1 — resumable video upload + processing**
...
**Группа 3 — федеративные, per-instance**
...
**Гейт:** если для Pixelfed потребовалось править исходники Mastodon — механизм диалектов (22) не
сделан, и дальше идти нельзя.

**Группа 4 — не-OAuth и своя единица счёта**

`bluesky`. Отдельная группа, потому что не делит механику ни с чем: клиент сам строит запись
...
**Группа 5 — gated access**

`linkedin` → `tiktok` → `x` → `pinterest`. Объединены не механикой, а тем, что блокер здесь не
код, а доступ: одобренные продукты LinkedIn, аудит приложения TikTok, платный tier X. Заявки
подаются **до группы 0**, параллельно с доработкой ядра. Механически: LinkedIn даёт `document`
...
- **После группы 0:** описать Mastodon и Bluesky на бумаге и убедиться, что обе ложатся без
  изменений ядра — свой хост инстанса, динамические лимиты, треды, idempotency key, alt text,
  чужая единица счёта текста, не-OAuth2 аутентификация. Дешевле узнать до группы 1, чем после
  восьмого адаптера.
- **После группы 3:** `pixelfed` получен без единой правки в исходниках Mastodon.
...
Правки у потребителя привязаны к группам адаптеров, а не к концу всего набора. Критерий один:
...
### Волна A — вместе с группой 0 (сделано)
...
### Волна B — перед группой 1, то есть перед первым OAuth

Колонки под токены (`accessToken`, `refreshToken`, `expiresAt`, `scopes`), `CredentialProvider`
поверх БД, обязательный `onCredentialsRefreshed`, обработка `AUTH_REFRESH_REQUIRED` как «канал
требует переавторизации», а не как ошибки публикации. Плюс окно ожидания обработки **на сеть**
вместо глобальных `MAX_STATUS_CHECKS = 30` и 15 минут: YouTube штатно выходит за 15 минут, и без
этого он будет «падать» на каждом длинном видео.

### Волна C — перед группой 2

Срок жизни подписанных media-URL: токен выпускается в момент публикации, а не замораживается в
`preparedPayload`, и живёт не меньше `urlMustRemainAvailableForSecs` из capability (пункт 15). Для
Telegram и Discord это незаметно, для pull-based Meta это провал публикации. Раньше делать
бессмысленно — до группы 2 в наборе нет ни одной pull-сети.
